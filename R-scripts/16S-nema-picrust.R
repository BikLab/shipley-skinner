...
ec_table <- read_excel("picrust/EC_data_nema.xlsx", sheet = "EC_Table")
ec_descrip <- read_excel("picrust/EC_data_nema.xlsx", sheet = "EC_Description")
ec_metadata <- read_excel("picrust/EC_data_nema.xlsx", sheet = "EC_Metadata")
...
set.seed(1)
phy_ec_nmds <- ordinate(
  physeq = phy_ec,
  method = "NMDS", 
  distance = "bray"
) # Stress: 0.1778909 

phy_ec_comp <- microbiome::transform(phy_ec, "compositional")
set.seed(1)
phy_ec_comp_nmds <- ordinate(
  physeq = phy_ec_comp,
  method = "NMDS", 
  distance = "bray"
) # Stress: 0.1464627

phy_ec_clr <- microbiome::transform(phy_ec, "clr")
set.seed(1)
phy_ec_clr_nmds <- ordinate(
  physeq = phy_ec_clr,
  method = "NMDS", 
  distance = "euclidean"
) # Stress: 0.2470883
...
ggsave("picrust/ec_phy_16S_nema_soil_type&habitat_no_transformation.pdf", width = 6, height = 4, dpi = 200)

######## Aldex2 analysis on phyoseq object ############
######## Taxonomy
aldex2_ec_phy_feeding <- ALDEx2::aldex(data.frame(phyloseq::otu_table(phy_ec)),
                                phyloseq::sample_data(phy_ec)$Feeding.Group,
                                mc.samples=128, test="kw", effect=TRUE, include.sample.summary=FALSE, denom="all", verbose=FALSE)

aldex2_ec_phy_infraorder <- ALDEx2::aldex(data.frame(phyloseq::otu_table(phy_ec)),
                                       phyloseq::sample_data(phy_ec)$Infraorder,
                                       mc.samples=128, test="kw", effect=TRUE, include.sample.summary=FALSE, denom="all", verbose=FALSE)

aldex2_ec_phy_family <- ALDEx2::aldex(data.frame(phyloseq::otu_table(phy_ec)),
                                          phyloseq::sample_data(phy_ec)$Family,
                                          mc.samples=128, test="kw", effect=TRUE, include.sample.summary=FALSE, denom="all", verbose=FALSE)

aldex2_ec_phy_genus <- ALDEx2::aldex(data.frame(phyloseq::otu_table(phy_ec)),
                                      phyloseq::sample_data(phy_ec)$Genus,
                                      mc.samples=128, test="kw", effect=TRUE, include.sample.summary=FALSE, denom="all", verbose=FALSE)
######## Environment
aldex2_ec_phy_site <- ALDEx2::aldex(data.frame(phyloseq::otu_table(phy_ec)),
                                     phyloseq::sample_data(phy_ec)$Sample.Site,
                                     mc.samples=128, test="kw", effect=TRUE, include.sample.summary=FALSE, denom="all", verbose=FALSE)

aldex2_ec_phy_habitat <- ALDEx2::aldex(data.frame(phyloseq::otu_table(phy_ec)),
                                     phyloseq::sample_data(phy_ec)$Habitat,
                                     mc.samples=128, test="kw", effect=TRUE, include.sample.summary=FALSE, denom="all", verbose=FALSE)

aldex2_ec_phy_soil_type <- ALDEx2::aldex(data.frame(phyloseq::otu_table(phy_ec)),
                                     phyloseq::sample_data(phy_ec)$Soil.Type,
                                     mc.samples=128, test="kw", effect=TRUE, include.sample.summary=FALSE, denom="all", verbose=FALSE)

# Write aldex2 results to file as csv
write.csv(aldex2_ec_phy_feeding, "picrust/aldex2_ec_phy_feeding.csv")
write.csv(aldex2_ec_phy_infraorder, "picrust/aldex2_ec_phy_infraorder.csv")
write.csv(aldex2_ec_phy_family, "picrust/aldex2_ec_phy_family.csv")
write.csv(aldex2_ec_phy_genus, "picrust/aldex2_ec_phy_genus.csv")
