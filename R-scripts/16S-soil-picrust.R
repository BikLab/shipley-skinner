...
library("DataCombine") #Tools for combining and cleaning data sets, particularly with grouped and time series data
...
ec_table <- read_excel("picrust/EC_data_soil.xlsx", sheet = "EC_Table")
ec_descrip <- read_excel("picrust/EC_data_soil.xlsx", sheet = "EC_Description")
ec_metadata <- read_excel("picrust/EC_data_soil.xlsx", sheet = "EC_Metadata")

# Convert asv and taxonomy tables to matrix format; sample table as dataframe
ec_table <- as.matrix(ec_table)
class(ec_table) # check the object class
ec_descrip <- as.matrix(ec_descrip)
ec_metadata <- as.data.frame(ec_metadata)
class(ec_metadata) # check the object class

# Rename row names using ASV column for asv table and taxonomy table; rename using Sample column for sample table
rownames(ec_table) <- ec_table[,1]
rownames(ec_descrip) <- ec_descrip[,1]
rownames(ec_metadata) <- ec_metadata[,1]

# Exclude first column from all three tables; set asv table as numeric
ec_table <- ec_table[,-1]
class(ec_table) <- "numeric"
#ec_descrip <- ec_descrip[,-1]
ec_metadata <- ec_metadata[,-1]

#Transform each table/matrix to phyloseq objects
ec_table_otu = otu_table(ec_table, taxa_are_rows = TRUE)
ec_descrip_tax = tax_table(ec_descrip)
ec_metadata_sample = sample_data(ec_metadata)

# Create a phyloseq object by combine all three previous objects
phy_ec <- phyloseq(ec_table_otu, ec_descrip_tax, ec_metadata_sample)
phy_ec #2440 taxa and 52 samples
head(sample_data(phy_ec)) # check the first 6 lines of your phyloseq object

#Visualize some data entries from the phyloseq object
sample_names(phy_ec) # list sample names
rank_names(phy_ec) # list the taxonomic ranks from your tax_table
sample_variables(phy_ec) # list the variables from your sample_data

# Check if there are samples with zero reads and remove them
smin <- min(sample_sums(phy_ec)) # smin = 0 means you have samples with no reads. Orignally sample number was 64732.07.

phy_ec_50_samples <- subset_samples(phy_ec,
                                    sample_names(phy_ec) != "SK.4.2" & # Low abundance Riparian sample
                                    sample_names(phy_ec) != "SK.6.3") # Low abundance Riparian sample
phy_ec_50_samples #2440 taxa and 50 samples
any(taxa_sums(phy_ec_50_samples) == 0) # if TRUE, there are ASVs with no reads, otherwise FALSE
sum(taxa_sums(phy_ec_50_samples) == 0) # gives the number of cases

# Ordination with nMDS remove low abundance samples
# no transformation
set.seed(1)
phy_ec_50_samples_nmds <- ordinate(
  physeq = phy_ec_50_samples,
  method = "NMDS", 
  distance = "bray"
) # Stress: 0.1151146

# compositional transformation
phy_ec_50_samples_comp <- microbiome::transform(phy_ec_50_samples, "compositional")
set.seed(1)
phy_ec_50_samples_comp_nmds <- ordinate(
  physeq = phy_ec_50_samples_comp,
  method = "NMDS", 
  distance = "bray"
) # Stress: 0.08430424

# clr transformation
phy_ec_50_samples_clr <- microbiome::transform(phy_ec_50_samples, "clr")
set.seed(1)
phy_ec_50_samples_clr_nmds <- ordinate(
  physeq = phy_ec_50_samples_clr,
  method = "NMDS", 
  distance = "euclidean"
) # Stress: 0.1549991

# hellinger transformation
phy_ec_50_samples_hel <- microbiome::transform(phy_ec_50_samples, "hellinger")
set.seed(1)
phy_ec_50_samples_hel_nmds <- ordinate(
  physeq = phy_ec_50_samples_hel,
  method = "NMDS", 
  distance = "bray"
) # Stress: 0.08098597
...
phyloseq::plot_ordination(
  physeq = phy_ec_50_samples_hel,
  ordination = phy_ec_50_samples_hel_nmds,
  type = "samples",
  color = "Habitat",
  shape = "Soil.Type") +
  ...
  annotate("text", x = 0.05, y = 0.04, label ="2D Stress: 0.08")
ggsave("picrust/ec_phy_16S_soil_soil_type&habitat_hellinger_transformation.pdf", width = 6, height = 4, dpi = 200)

######## Aldex2 analysis on phyoseq object ############
######## Environment
aldex2_soil_ec_phy_site <- ALDEx2::aldex(data.frame(phyloseq::otu_table(phy_ec_50_samples)),
                                       phyloseq::sample_data(phy_ec_50_samples)$Sample.Site,
                                       mc.samples=128, test="kw", effect=TRUE, include.sample.summary=FALSE, denom="all", verbose=FALSE)

aldex2_soil_ec_phy_habitat <- ALDEx2::aldex(data.frame(phyloseq::otu_table(phy_ec_50_samples)),
                                phyloseq::sample_data(phy_ec_50_samples)$Habitat,
                                mc.samples=128, test="kw", effect=TRUE, include.sample.summary=FALSE, denom="all", verbose=FALSE)

aldex2_soil_phy_soil_type <- ALDEx2::aldex(data.frame(phyloseq::otu_table(phy_ec_50_samples)),
                                       phyloseq::sample_data(phy_ec_50_samples)$Soil.Type,
                                       mc.samples=128, test="kw", effect=TRUE, include.sample.summary=FALSE, denom="all", verbose=FALSE)

# Write aldex2 results to file as csv
write.csv(aldex2_ec_phy_site, "picrust/aldex2_ec_phy_site.csv")
write.csv(aldex2_ec_phy_habitat, "picrust/aldex2_ec_phy_habitat.csv")
write.csv(aldex2_ec_phy_soil_type, "picrust/aldex2_ec_phy_soil_type.csv")

# Import aldex2 results and rename the X variable by OTU
shsk_soil_ko_aldex_soil_type_result <- read.csv("picrust/shsk_soil_ko_aldex_soil_type_res.csv")
shsk_soil_ko_aldex_habitat_result <- read.csv("picrust/shsk_soil_ko_aldex_habitat_res.csv")

aldex2_ec_phy_feeding_result <- read.csv("picrust/aldex2_ec_phy_feeding.csv")
colnames(aldex2_ec_phy_feeding_result) <- c("EC_Number", "kw.ep", "kw.eBH", "glm.ep", "glm.eBH")

aldex2_ec_phy_infraorder_result <- read.csv("picrust/aldex2_ec_phy_infraorder.csv")
colnames(aldex2_ec_phy_infraorder_result) <- c("EC_Number", "kw.ep", "kw.eBH", "glm.ep", "glm.eBH")

aldex2_ec_phy_family_result <- read.csv("picrust/aldex2_ec_phy_family.csv")
colnames(aldex2_ec_phy_family_result) <- c("EC_Number", "kw.ep", "kw.eBH", "glm.ep", "glm.eBH")

aldex2_ec_phy_genus_result <- read.csv("picrust/aldex2_ec_phy_genus.csv")
colnames(aldex2_ec_phy_genus_result) <- c("EC_Number", "kw.ep", "kw.eBH", "glm.ep", "glm.eBH")

aldex2_ec_phy_site_result <- read.csv("picrust/aldex2_ec_phy_site.csv")
colnames(aldex2_ec_phy_site_result) <- c("EC_Number", "kw.ep", "kw.eBH", "glm.ep", "glm.eBH")

aldex2_ec_phy_habitat_result <- read.csv("picrust/aldex2_ec_phy_habitat.csv")
colnames(aldex2_ec_phy_habitat_result) <- c("EC_Number", "kw.ep", "kw.eBH", "glm.ep", "glm.eBH")

aldex2_ec_phy_soil_type_result <- read.csv("picrust/aldex2_ec_phy_soil_type.csv")
colnames(aldex2_ec_phy_soil_type_result) <- c("EC_Number", "kw.ep", "kw.eBH", "glm.ep", "glm.eBH")

#Clean up presentation
ec_abundance <- read_excel("picrust/EC_data_nema.xlsx", sheet = "EC_Table")
ec_function <- read_excel("picrust/EC_data_nema.xlsx", sheet = "EC_Description")
ec_factors <- read_excel("picrust/EC_data_nema.xlsx", sheet = "EC_Metadata")

ko_abundance <- read_excel("picrust/ko_data_soil.xlsx", sheet = "ko_Table")
ko_function <- read_excel("picrust/ko_data_soil.xlsx", sheet = "ko_Description")
ko_factors <- read_excel("picrust/ko_data_soil.xlsx", sheet = "ko_Metadata")

# filter aldex2 results by sig kw.ep and join the taxanomic information
shsk_soil_ko_aldex_soil_type_result_sig <- shsk_soil_ko_aldex_soil_type_result %>%
  filter(kw.ep < 0.05) %>%
  arrange(kw.ep, kw.eBH) %>%
  select(KO_number, kw.ep, kw.eBH)
shsk_soil_ko_aldex_soil_type_result_sig <- left_join(shsk_soil_ko_aldex_soil_type_result_sig, ko_function)
write.csv(shsk_soil_ko_aldex_soil_type_result_sig, "picrust/shsk_soil_ko_aldex_soil_type_result_sig.csv")

shsk_soil_ko_aldex_habitat_result_sig <- shsk_soil_ko_aldex_habitat_result %>%
  filter(kw.ep < 0.05) %>%
  arrange(kw.ep, kw.eBH) %>%
  select(KO_number, kw.ep, kw.eBH)
shsk_soil_ko_aldex_habitat_result_sig <- left_join(shsk_soil_ko_aldex_habitat_result_sig, ko_function)
write.csv(shsk_soil_ko_aldex_habitat_result_sig, "picrust/shsk_soil_ko_aldex_habitat_result_sig.csv")

aldex2_ec_phy_feeding_result_sig <- aldex2_ec_phy_feeding_result %>%
  filter(kw.ep < 0.05) %>%
  arrange(kw.ep, kw.eBH) %>%
  dplyr::select(EC_Number, kw.ep, kw.eBH)
aldex2_ec_phy_feeding_result_sig <- left_join(aldex2_ec_phy_feeding_result_sig, ec_function)
write.csv(aldex2_ec_phy_feeding_result_sig, "picrust/aldex2_ec_phy_feeding_result_sig.csv")

aldex2_ec_phy_infraorder_result_sig <- aldex2_ec_phy_infraorder_result %>%
  filter(kw.ep < 0.05) %>%
  arrange(kw.ep, kw.eBH) %>%
  dplyr::select(EC_Number, kw.ep, kw.eBH)
aldex2_ec_phy_infraorder_result_sig <- left_join(aldex2_ec_phy_infraorder_result_sig, ec_function)
write.csv(aldex2_ec_phy_infraorder_result_sig, "picrust/aldex2_ec_phy_infraorder_result_sig.csv")

aldex2_ec_phy_family_result_sig <- aldex2_ec_phy_family_result %>%
  filter(kw.ep < 0.05) %>%
  arrange(kw.ep, kw.eBH) %>%
  dplyr::select(EC_Number, kw.ep, kw.eBH)
aldex2_ec_phy_family_result_sig <- left_join(aldex2_ec_phy_family_result_sig, ec_function)
write.csv(aldex2_ec_phy_family_result_sig, "picrust/aldex2_ec_phy_family_result_sig.csv")

aldex2_ec_phy_genus_result_sig <- aldex2_ec_phy_genus_result %>%
  filter(kw.ep < 0.05) %>%
  arrange(kw.ep, kw.eBH) %>%
  dplyr::select(EC_Number, kw.ep, kw.eBH)
aldex2_ec_phy_genus_result_sig <- left_join(aldex2_ec_phy_genus_result_sig, ec_function)
write.csv(aldex2_ec_phy_genus_result_sig, "picrust/aldex2_ec_phy_genus_result_sig.csv")

aldex2_ec_phy_site_result_sig <- aldex2_ec_phy_site_result %>%
  filter(kw.ep < 0.05) %>%
  arrange(kw.ep, kw.eBH) %>%
  dplyr::select(EC_Number, kw.ep, kw.eBH)
aldex2_ec_phy_site_result_sig <- left_join(aldex2_ec_phy_site_result_sig, ec_function)
write.csv(aldex2_ec_phy_site_result_sig, "picrust/aldex2_ec_phy_site_result_sig.csv")

aldex2_ec_phy_habitat_result_sig <- aldex2_ec_phy_habitat_result %>%
  filter(kw.ep < 0.05) %>%
  arrange(kw.ep, kw.eBH) %>%
  dplyr::select(EC_Number, kw.ep, kw.eBH)
aldex2_ec_phy_habitat_result_sig <- left_join(aldex2_ec_phy_habitat_result_sig, ec_function)
write.csv(aldex2_ec_phy_habitat_result_sig, "picrust/aldex2_ec_phy_habitat_result_sig.csv")

aldex2_ec_phy_soil_type_result_sig <- aldex2_ec_phy_soil_type_result %>%
  filter(kw.ep < 0.05) %>%
  arrange(kw.ep, kw.eBH) %>%
  dplyr::select(EC_Number, kw.ep, kw.eBH)
aldex2_ec_phy_soil_type_result_sig <- left_join(aldex2_ec_phy_soil_type_result_sig, ec_function)
write.csv(aldex2_ec_phy_soil_type_result_sig, "picrust/aldex2_ec_phy_soil_type_result_sig.csv")


# Create clr objects by using EC_Number ids and original EC table, all significant ECs
aldex2_ko_phy_soil_type_result_sig_count <- left_join(shsk_soil_ko_aldex_soil_type_result_sig, ko_abundance)
write.csv(aldex2_ko_phy_soil_type_result_sig_count, "picrust/aldex2_ko_phy_soil_type_result_sig_count.csv")

aldex2_ko_phy_habitat_result_sig_count <- left_join(shsk_soil_ko_aldex_habitat_result_sig, ko_abundance)
write.csv(aldex2_ko_phy_habitat_result_sig_count, "picrust/aldex2_ko_phy_habitat_result_sig_count.csv")


aldex2_ec_phy_feeding_result_sig_count <- left_join(aldex2_ec_phy_feeding_result_sig, ec_abundance)
write.csv(aldex2_ec_phy_feeding_result_sig_count, "picrust/aldex2_ec_phy_feeding_result_sig_count.csv")

aldex2_ec_phy_infraorder_result_sig_count <- left_join(aldex2_ec_phy_infraorder_result_sig, ec_abundance)
write.csv(aldex2_ec_phy_infraorder_result_sig_count, "picrust/aldex2_ec_phy_infraorder_result_sig_count.csv")

aldex2_ec_phy_family_result_sig_count <- left_join(aldex2_ec_phy_family_result_sig, ec_abundance)
write.csv(aldex2_ec_phy_family_result_sig_count, "picrust/aldex2_ec_phy_family_result_sig_count.csv")

aldex2_ec_phy_genus_result_sig_count <- left_join(aldex2_ec_phy_genus_result_sig, ec_abundance)
write.csv(aldex2_ec_phy_genus_result_sig_count, "picrust/aldex2_ec_phy_genus_result_sig_count.csv")

aldex2_ec_phy_site_result_sig_count <- left_join(aldex2_ec_phy_site_result_sig, ec_abundance)
write.csv(aldex2_ec_phy_site_result_sig_count, "picrust/aldex2_ec_phy_site_result_sig_count.csv")

aldex2_ec_phy_habitat_result_sig_count <- left_join(aldex2_ec_phy_habitat_result_sig, ec_abundance)
write.csv(aldex2_ec_phy_habitat_result_sig_count, "picrust/aldex2_ec_phy_habitat_result_sig_count.csv")

aldex2_ec_phy_soil_type_result_sig_count <- left_join(aldex2_ec_phy_soil_type_result_sig, ec_abundance)
write.csv(aldex2_ec_phy_soil_type_result_sig_count, "picrust/aldex2_ec_phy_soil_type_result_sig_count.csv")

clr_feeding <- aldex2_ec_phy_feeding_result_sig_count[, -(2:4)]
rownames(clr_feeding) <- clr_feeding$EC_Number
clr_feeding <- clr_feeding[, -1]

clr_infraorder <- aldex2_ec_phy_infraorder_result_sig_count[, -(2:4)]
rownames(clr_infraorder) <- clr_infraorder$EC_Number
clr_infraorder <- clr_infraorder[, -1]

clr_family <- aldex2_ec_phy_family_result_sig_count[, -(2:4)]
rownames(clr_family) <- clr_family$EC_Number
clr_family <- clr_family[, -1]

clr_genus <- aldex2_ec_phy_genus_result_sig_count[, -(2:4)]
rownames(clr_genus) <- clr_genus$EC_Number
clr_genus <- clr_genus[, -1]

clr_site <- aldex2_ec_phy_site_result_sig_count[, -(2:4)]
rownames(clr_site) <- clr_site$EC_Number
clr_site <- clr_site[, -1]

clr_habitat <- aldex2_ec_phy_habitat_result_sig_count[, -(2:4)]
rownames(clr_habitat) <- clr_habitat$EC_Number
clr_habitat <- clr_habitat[, -1]

clr_soil_type <- aldex2_ec_phy_soil_type_result_sig_count[, -(2:4)]
rownames(clr_soil_type) <- clr_soil_type$EC_Number
clr_soil_type <- clr_soil_type[, -1]

# Adjusting zeros on the matrix and applying log transformation
czm_feeding <- cmultRepl(t(clr_feeding),  label=0, method="CZM")
czm_feeding_clr <- t(apply(czm_feeding, 1, function(x){log(x) - mean(log(x))}))
czm_infraorder <- cmultRepl(t(clr_infraorder),  label=0, method="CZM")
czm_infraorder_clr <- t(apply(czm_infraorder, 1, function(x){log(x) - mean(log(x))}))
czm_family <- cmultRepl(t(clr_family),  label=0, method="CZM")
czm_family_clr <- t(apply(czm_family, 1, function(x){log(x) - mean(log(x))}))
czm_genus <- cmultRepl(t(clr_genus),  label=0, method="CZM")
czm_genus_clr <- t(apply(czm_genus, 1, function(x){log(x) - mean(log(x))}))
czm_site <- cmultRepl(t(clr_site),  label=0, method="CZM")
czm_site_clr <- t(apply(czm_site, 1, function(x){log(x) - mean(log(x))}))
czm_habitat <- cmultRepl(t(clr_habitat),  label=0, method="CZM")
czm_habitat_clr <- t(apply(czm_habitat, 1, function(x){log(x) - mean(log(x))}))
czm_soil_type <- cmultRepl(t(clr_soil_type),  label=0, method="CZM")
czm_soil_type_clr <- t(apply(czm_soil_type, 1, function(x){log(x) - mean(log(x))}))
...
Z.Score_czm_feeding_clr <- scale(t(czm_feeding_clr))
Z.Score_czm_infraorder_clr <- scale(t(czm_infraorder_clr))
Z.Score_czm_family_clr <- scale(t(czm_family_clr))
Z.Score_czm_genus_clr <- scale(t(czm_genus_clr))
Z.Score_czm_site_clr <- scale(t(czm_site_clr))
Z.Score_czm_habitat_clr <- scale(t(czm_habitat_clr))
Z.Score_czm_soil_type_clr <- scale(t(czm_soil_type_clr))
...
ha_top = HeatmapAnnotation(
  Soil = as.vector(ec_factors$Soil.Type),
  Habitat = as.vector(ec_factors$Habitat),
  Feeding = as.vector(ec_factors$Feeding.Group),
  col = list(
    ...
    Feeding = c("Bacterivores" = "#1B9E77", "Fungivores" = "#D95F02","Herbivores" = "#7570B3",
                "Omnivores" = "#E7298A", "Predator" = "#E6AB02", "unknown" = "#666666")
    ),
  annotation_legend_param = list(
    ...
    Feeding = list(
      title = "Feeding",
      at = c("Bacterivores", "Fungivores", "Herbivores", "Omnivores", "Predator", "unknown"),
      labels = c ("BACT", "FUNG", "HERB", "OMNI", "PRED", "UNKN")
      
    )
  ))

# Organize total abundance and EC function name
Z.Score_czm_feeding_clr_name <- as.data.frame(Z.Score_czm_feeding_clr)
str(Z.Score_czm_feeding_clr_name)
Z.Score_czm_feeding_clr_name <- rownames_to_column(Z.Score_czm_feeding_clr_name, var = "EC_Number")
Z.Score_czm_feeding_clr_name <- left_join(Z.Score_czm_feeding_clr_name, ec_function)
head(Z.Score_czm_feeding_clr_name)
Z.Score_czm_feeding_clr_name <- Z.Score_czm_feeding_clr_name[, -(2:521)]

feeding_ec_table_total <- as.data.frame(ec_abundance)
feeding_ec_table_total$Total <- rowSums(feeding_ec_table_total[, -1])
head(feeding_ec_table_total)
feeding_ec_table_total <- feeding_ec_table_total[, -(2:521)]

Z.Score_czm_feeding_count_total <- left_join(Z.Score_czm_feeding_clr_name, feeding_ec_table_total)
head(Z.Score_czm_feeding_count_total)

# Defining color scheme for row annotations
abundance_col_fun = colorRamp2(c(100000, 1000000, 10000000, 100000000),
                               c("#c7eae5",
                                 "#80cdc1",
                                 "#35978f",
                                 "#01665e"))
...
ha_right_ec = rowAnnotation(
  Abundance = Z.Score_czm_feeding_count_total$Total, border = FALSE, col = list(Abundance = abundance_col_fun))
row_labels_ec = Z.Score_czm_feeding_count_total$EC_Description

row_labels_phy = Z.Score.phylum_hab_count_total$Phylum

# Plot heatmap at the phylum level
hm_ec_feeding <- Heatmap(Z.Score_czm_feeding_clr, name = "Z-score, CLR", col = col_matrix,
                         column_title = "EC Predicted Functions", 
                         column_title_gp = gpar(fontface = "bold", fontsize = 14),
                         column_split = as.vector(as.vector(ec_factors$Soil.Type)),
...
# Subset by sample types, that is, only soil samples removing controls and blanks since we already filtered potential contaminants
